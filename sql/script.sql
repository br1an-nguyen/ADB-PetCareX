...
CREATE PROCEDURE sp_NhanVien_TaoPhieuKhamTrucTiep
    @ID_NhanVien char(10),    -- Nhân viên đang trực quầy
    @ID_ThuCung char(10),     -- Thú cưng được chọn
    @ID_DichVuGoc char(10)    -- Dịch vụ khách yêu cầu (VD: DV001)
AS
BEGIN
    SET NOCOUNT ON;
    BEGIN TRANSACTION;
    BEGIN TRY
        ...
        SELECT @ID_ChiNhanh = ID_ChiNhanh FROM NhanVien WHERE ID_NhanVien = @ID_NhanVien;

        -- 2. Lấy ID dịch vụ chi nhánh (kèm giá tiền)
        DECLARE @ID_DichVuChiNhanh char(10);
        DECLARE @GiaTien float;
        
        SELECT @ID_DichVuChiNhanh = ID_DichVuDuocDung, @GiaTien = Gia_DichVu
        FROM ChiNhanh_DichVu 
        WHERE ID_ChiNhanh = @ID_ChiNhanh AND ID_DichVu = @ID_DichVuGoc;

        IF @ID_DichVuChiNhanh IS NULL
        BEGIN
            RAISERROR(N'Chi nhánh hiện tại của bạn không có dịch vụ này.', 16, 1);
            ROLLBACK TRANSACTION; RETURN;
        END

        -- 3. Lấy thông tin chủ thú cưng để gắn vào hóa đơn
        DECLARE @ID_ChuSoHuu char(10);
        SELECT @ID_ChuSoHuu = ID_TaiKhoan FROM ThuCung WHERE ID_ThuCung = @ID_ThuCung;
        ...
        VALUES (@ID_HoaDonMoi, GETDATE(), @ID_NhanVien, @ID_ChuSoHuu, 'TT001', @GiaTien); 
        -- Mặc định TT001 là tiền mặt, có thể update sau
        ...
        VALUES (dbo.TaoIDPhieuKham(), @ID_HoaDonMoi, @ID_DichVuChiNhanh, @ID_ThuCung, N'Chờ khám', GETDATE());

        COMMIT TRANSACTION;
        PRINT N'Đã tạo phiếu khám thành công. Số thứ tự: ' + @ID_HoaDonMoi;
    END TRY
    BEGIN CATCH
